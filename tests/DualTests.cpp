...
TEST_SUITE("Dual number tests")
{
    TEST_CASE("Construct dual numbers")
    {
        ...
        {
            d = toVariable(static_cast<double>(5.0));
            expectedValue = 5.0;
        ...
        {
            d = toConstant(static_cast<double>(5.0));
            expectedValue = 5.0;
            expectedDerivative = 0.0;
        }

        CHECK(d.value() == expectedValue);
        CHECK(d.derivative() == expectedDerivative);
    }

    TEST_CASE("Unary operations")
    {
        constexpr Dual<double> d(3.0, 2.0);
    ...
    {
        constexpr Dual<double> d(4.0, 2.0);
        double scalar = 5.0;
        Dual<double> result1, expected;
